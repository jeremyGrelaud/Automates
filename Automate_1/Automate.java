...
public class Automate {
	private List<Etat> etats; /*liste des differents etats de l'automate*/
	...
	public boolean containsEtats(final String la_chaine) {  //faut check les nom des etats de la liste et voir si c'est dedans
		//ca devrait etre le nbr de transis associees a cet etat
    ...
    public Etat getEtatByNom(String id) {
        ...
        return null; // si l'etat n'est pas dans l automate
    }*/
	...
	public boolean est_standard() { //il faut une seule entr�e et aucune transi vers cette entr�e
		...
		if(compteur!=1) {
			...
			//verifier qu'aucune transi ne mene a cet entree
			String id = new_etat.getNom();
			//aucune transi ne doit avoir comme etat d'arrivee un etat.getNom() == id
			...
			//a la fin de la boucle si rien a ete return alors l'automate est standard
			return true;
		}
	}
	...
	public boolean est_un_automate_complet() {
		...
		if((automate.est_un_automate_asynchrone()==false)) { //&& (automate.est_un_automate_deterministe()==true)) {
			...
			System.out.println("\nVotre automate n'est pas synchrone et deterministe il ne peut pas etre complet");
			return false;
		}
	}
	...
	//il faudrait indiquer a partir de quel type d'automate on obtient le compelmentaire : cad s'il etait minimal ou non
	public Automate automate_complementaire() {
		
		Automate automate = new Automate(this);
		
		if(!automate.est_un_automate_deterministe() || !automate.est_un_automate_complet()) {
			System.out.println("Impossible d'avoir le complementaire l'automate n'est pas d�terminsite et compelt");
			...
			//il faut s'assurer de faire pointer les etats sur les nouveaux dans la table de transition
			
			for(Etat etat : this.getEtats()) {
				for(String clef : etat.getTransi().keySet()) {
					for(Etat etat_destination : etat.getTransi().get(clef)) {
						for(Etat etat_origine : this.getEtats() ) {
							//on cherche a remettre le bon etat
							if((etat_destination.getNom()).equals(etat_origine.getNom())) {
								etat_destination.setTransi(etat_origine.getTransi()); 
								etat_destination.setTypes(etat_origine.getTypes());
							}
						}
					}
				}
	...
	public void programme() {
		...
		do {
			...
			if(mot.equals("3")){
				automate_a_modifier = automate_a_modifier.automate_complementaire();
			...
			if(mot.equals("5")){
				automate_a_modifier.determinisation(automate_a_modifier);
