...
public class Automate {
	...
	public Etat getEtatByNom(final String id) { //on veut recup un etat specifique de l automate avec son id
		//on parcours la liste jusqu'ï¿½ trouver l'etat correspondant
	...
	/*** STANDARDISATION*/
	public boolean est_standard() { //il faut une seule entrï¿½e et aucune transi vers cette entrï¿½e
		...
		if(est_standard()) {
			...
			*/
			...
			for(int i=0; i<automate.getEtats().size();i++) {
				automate.etats.get(i).getTypes().remove(TypeEtat.ENTRY);
			}
	...
	public boolean est_un_automate_asynchrone() {
		...
		for(Etat etat : automate.etats) {
			for(String clef : etat.getTransi().keySet()) { //string avec tt les clefs
				...
				if(clef == "") {
					System.out.println("\nL'automate est asynchrone car :\n"); //va ï¿½tre affichï¿½ plusieurs fois
					System.out.println(etat.getNom() +" - "+ epsilon+" - "+etat.getTransi().get(clef).toString());
					compteur++;
	...
	public boolean est_un_automate_deterministe() { //1 seule entrï¿½e et liste pour une clef = taille 1 pas plus
		...
		if(compteur_entree>1) {
			System.out.println("\nNon dï¿½terministe car plus d'une entrï¿½e");
			return false;
		}
		...
		for(Etat etat : this.etats) {
			for(String clef : etat.getTransi().keySet()) { //string avec tt les clefs
				if(etat.getTransi().get(clef).size()>1) {
					System.out.println("\nNon dï¿½terministe car on a la transi : "+etat.getNom()+"-"+clef+"-"+etat.getTransi().get(clef).toString());
					return false;
				}
			} 
		}
		System.out.println("\nL'automate est dï¿½terministe");
		return true;
	}
	...
	*/
	...
	public Automate determinisation_et_completion_synchrone(Automate automate) {
		...
		//s'il y a plusieurs entrï¿½es on va devoir les fusionner en une nouvelle entrï¿½e
		Etat new_entry = new Etat(liste_entrees.get(0));
		...
		for(int i=1;i<liste_entrees.size();i++) {
			...
			String ancien_nom2 = liste_entrees.get(i).getNom();
			new_entry.setNom(new_entry.getNom() + liste_entrees.get(i).getNom());
			//il faudrait aussi changer les noms des transis associï¿½es
		...
		*/
		...
		do {
			...
			for(String clef : new_entry.getTransi().keySet() ) {
				if(new_entry.getTransi().get(clef).size()>1) {
					//fusion faudrait fusionner 2 a 2 et rappeler ï¿½a jusqu'a ce que la taille de la liste = 1
					Etat nouvel_etat;
					do {
		...
		//on a l'entrï¿½e maintenant on va gï¿½nï¿½rer les nouvelles transitions
		
		//on regarde les etats d'arrivï¿½ qu'on a pour notre entrï¿½e
		//si ces etats ne sont pas dans notre liste d'ï¿½tat alors on crï¿½er de nouveaux ï¿½tats
		//on rï¿½cup ses transis avec l'ancien automate
		//on ajoute ï¿½ notre table
		//et nouveau tour de boucle on regarde s'il y a des nouveaux ï¿½tats 
		//etc
		
		
		//complï¿½ter avant de renvoyer 
		return automate_deter_sync;
	}
	...
	public boolean est_un_automate_complet() {
		...
		if((automate.est_un_automate_asynchrone()==false)) { //&& (automate.est_un_automate_deterministe()==true)) {
			//check si chaque etat possï¿½de le bon nbr de clefs
			int compteur=0;
			for(Etat etat : this.etats) {
				for(String clef : etat.getTransi().keySet()) { 
						compteur++;
			...
			System.out.println("\nVotre automate n'est pas synchrone et dï¿½terministe il ne peut pas ï¿½tre complet");
			return false;
		}
	}
	...
	public Automate completion() {
		...
		//on rajoute le nouvel etat p ï¿½ l'automate
		//automate.etats.add(p);
		this.etats.add(p);
	...
	/***MINIMISATION*/
	//doit afficher si c'ï¿½tait dï¿½jï¿½ minimal
	//on minimise un automate synchrone, dï¿½terministe, complet
	...
	*/
	...
	public boolean reconnaitre_mot_automate_determinsite ( String mot ) {
		...
		if(!this.est_un_automate_deterministe()) {
			System.out.println("Impossible de vï¿½rifier votre automate n'est pas dï¿½terminsite");
			return false;
		}
		else {	
			Etat etat_courant = null; /*etat de dï¿½part de l'algo*/
			for(Etat etat : this.etats) {
				if(etat.getTypes().contains(TypeEtat.ENTRY)) {
					 etat_courant = etat;
				}
			}
			/*il faudrait un appel recursif avec toutes les entrÃ©es*/
		
			/*il pourrait y avoir plusieurs entrï¿½es si pas dï¿½terministe*/
			...
			do {
				...
				/*si on est ï¿½ la fin du mot et que la lettre correspond ï¿½ la denriï¿½re lettre du mot et qu'on est sur une sortie c bon*/
				...
				if(existe_transi(etat_courant, symbole_futur)) {
					...
					/*faudrait un appel rÃ©cursif avec tous les Ã©tats*/
			...
			if(etat.getTypes().contains(TypeEtat.ENTRY)) {
				...
				while(continuer && i<mot.length()) {
					if(new_entry.getTransi().get(String.valueOf(mot.charAt(i)))!=null) {
						...
						//new_entry = new_entry.getTransi().get(String.valueOf(mot.charAt(i)));
						//il faudrait faire une fc rï¿½cursive qui appel avec tous les ï¿½tats terminaux de la liste
						//puis quand ï¿½a arrive ï¿½ la fin du mot faut regarder si c'est un ï¿½tat de sortie
						i++;
					}
					else {
						continuer = false;
	...
	*/
	...
	//il faudrait indiquer ï¿½ partir de quel type d'automate on obtient le compï¿½lmentaire : cad s'il ï¿½tait minimal ou non
	public Automate automate_complementaire(Automate automate) {
		if(!automate.est_un_automate_deterministe() && !automate.est_un_automate_complet()) {
			System.out.println("Impossible d'avoir le complï¿½mentaire l'automate n'est pas dï¿½terminsite et compelt");
			System.out.println("Automate renvoyï¿½ non modifiï¿½");
			return automate;
		}
		else {
			Automate automate_complementaire = new Automate(automate);
			...
			System.out.println("Automate complï¿½mentaire crï¿½ï¿½");
			return automate_complementaire;
		}
